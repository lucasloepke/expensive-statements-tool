...
MDS_KEYS = ['START_TIME', 'USER', 'STATEMENT_HASH', 'APPLICATION_NAME', 'TENANT_ID', 'DURATION_S', 'CPU_TIME_S', 'PARALLEL_FACTOR', 'MEMORY_SIZE', 'MODEL_ID', 'MODEL_NAME', 'STORY_ID', 'STORY_NAME', 'WIDGET_ID', 'MDS_TYPE', 'DIMENSIONS', 'READ_MODE', 'MEASURES', 'MEASURE_TYPE', 'INPUT_RECORDS']
...
def get_mds_metadata(statement):
    ...
    if 'Analytics' in stmt_json:
        if 'DataSource' in stmt_json['Analytics']:
...
def get_stmt_type(statement):
    if statement.startswith('CALL SYS.EXECUTE_MDS'):
        ...
        return "EPM_BACKUP"
    elif 'dataUpdateTemp' in statement:
        return "Wrangling"
    else:
        return "OTHER"
...
if __name__ == "__main__":
    ...
    print("Opening File Selector - Statements")
    ...
    summary_out_file = file_name.split('.csv')[0] + "_summary.xlsx"
    ...
    prep_duration = end_prep_time-t1
    print(f"Preperation Time: {prep_duration:.2f} seconds")
    ...
    for row in file:
        ...
        summary_row['SCHEMA_NAME'] = row['SCHEMA_NAME']
        summary_row['TENANT_ID'] = row['APP_USER'][0] if row['APP_USER'] else ""
    ...
    summary_duration = end_summary-t1
    print(f"Summary Time: {summary_duration:.2f} seconds")

    summary_out = copy.deepcopy(summary_list)
    ...
    end_mds = time.time()
    mds_duration = end_mds-end_summary
    print(f"MDS Time: {mds_duration:.2f} seconds")
    ...
    for statement in summary_list:
        ...
        if statement['USER'] not in user_action_mapping:
            unidentified_actions.append(statement)
            continue
        ...
        for action in user_actions:
            ...
            if action['INTERACTION_NAME'] in EPM_INTERNAL_ACTIONS:
                ...
                if "dataAction" in da:
                    ...
                    action_metadata['ACTION_NAME'] = da['value']
    ...
    action_duration = end_action-end_mds
    print(f"Action Time: {action_duration:.2f} seconds")


    summary_df = pd.DataFrame(summary_out)
    mds_df = pd.DataFrame(mds_summary)
    da_df = pd.DataFrame(da_summary)

    # file_name = 'C:/Users/frank.naylor@sap.com/OneDrive - SAP SE/2024/Mondelez/Monitoring/May/Q3/513_516/Q3_513_516_statements_summary.xlsx'

    with pd.ExcelWriter(summary_out_file, engine='openpyxl') as writer:
        summary_df.to_excel(writer, sheet_name='Summary', index=False)
        mds_df.to_excel(writer, sheet_name='MDS Summary', index=False)
        da_df.to_excel(writer, sheet_name='Action Summary',index=False)

    end_post = time.time()
    post_duration = end_post - end_action
    print(f"Post Process Time: {post_duration:.2f} seconds")

    end_time = time.time()
    total_duration = end_time - t1
    print(f"Post Process Time: {total_duration:.2f} seconds")
